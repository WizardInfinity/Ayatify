/**
 * Ayatify - Data batas Juz
 * Data statis pembagian 30 Juz Al-Qur'an (nomor surah & ayat awal/akhir tiap juz).
 * Data ini tetap (fixed) sesuai mushaf standar, dipakai untuk memotong rentang
 * ayat dari data surah yang diambil lewat API per-surah.
 */
const JUZ_DATA = [
...
/** Mengembalikan daftar {surah, start, end} yang perlu diambil untuk satu nomor juz */
function getSurahRangesForJuz(juzNumber) {
  const j = JUZ_DATA.find((x) => x.juz === juzNumber);
  if (!j) return [];
  const ranges = [];
  if (j.startSurah === j.endSurah) {
    ranges.push({ surah: j.startSurah, start: j.startAyah, end: j.endAyah });
  } else {
    ranges.push({ surah: j.startSurah, start: j.startAyah, end: null });
    for (let s = j.startSurah + 1; s < j.endSurah; s++) {
      ranges.push({ surah: s, start: 1, end: null });
    }
    ranges.push({ surah: j.endSurah, start: 1, end: j.endAyah });
  }
  return ranges;
}
